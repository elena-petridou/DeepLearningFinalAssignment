...
from tensorflow.keras.layers import Dense, Flatten, Conv2D, Conv2DTranspose, Reshape, MaxPooling2D, BatchNormalization, LeakyReLU
...
def load_real_samples(scale=False):
    X = np.load("new_cards.npy")
    X = ImageConverter.remove_black_images(X, thresh=170)
...
print(dataset.shape)
grid_plot(dataset[np.random.randint(0, 1000, 9)], name='Cards dataset (64x64x3)', n=3)
...
def build_convolutional_autoencoder(data_shape, latent_dim, filters=128):
    ...
    autoencoder.compile(loss='mse', optimizer=tf.keras.optimizers.Adam(learning_rate=1e-4))
...
for epoch in range(30):
    ...
    cae.fit(x=dataset, y=dataset, epochs=1, batch_size=8)
...
class KLWarmup(tf.keras.callbacks.Callback):
    def __init__(self, kl_weight_var, warmup_epochs, max_kl_weight = 1.0):
        super().__init__()
        self.kl_weight_var = kl_weight_var  # Store the TF Variable
        self.warmup_epochs = warmup_epochs
        self.max_kl_weight = max_kl_weight

    def on_epoch_begin(self, epoch, logs=None):
        # Compute new weight (epoch starts at 0, so epoch 0 gives 0.0)
        new_weight = min(epoch / self.warmup_epochs, self.max_kl_weight) 
        # Update the TensorFlow variable
        self.kl_weight_var.assign(new_weight)

    def on_epoch_end(self, epoch, logs=None):
        # Log the current value
        current = float(self.kl_weight_var.numpy())
        # Make it appear in fit() logs
        logs["kl_weight"] = current

class ImageGeneration(tf.keras.callbacks.Callback):
    def __init__(self, decoder, latent_dim, coefficient=4, n_images=9):
        super().__init__()
    ...
    def on_epoch_end(self, epoch, logs=None):
        ...
        latent_vectors = np.random.randn(self.n_images, self.latent_dim)
        # Feed to decoder
        images = self.decoder(latent_vectors / self.coefficient)
        # Plot
        grid_plot(images, epoch, name='VAE generated images (randomly sampled from the latent space)', n=3, save=False)


def build_vae(data_shape, latent_dim, filters=128, warm_up_epochs=10, max_kl_weight = 1.0):
    
    # Create a variable for the KL weight that will be updated during training
    # trainable=False because we don't want the optimizer to update it
    kl_weight_var = tf.Variable(0.0, trainable=False, dtype=tf.float32, name="kl_weight")
    
    # Building the encoder
    #encoder = build_conv_net(data_shape, latent_dim*2, filters=filters, n_downsampling_layers=4)
    encoder = tf.keras.models.Sequential(layers=[
        ...
        BatchNormalization(),
        LeakyReLU(0.2),
        Conv2D(filters=filters*2, kernel_size = 3, strides = 2),
        BatchNormalization(),
        LeakyReLU(0.2),
        Conv2D(filters=filters*2, kernel_size = 3, strides = 2),
        BatchNormalization(),
        LeakyReLU(0.2),
        Flatten(),
        Dense(units=latent_dim*4, kernel_initializer="normal", bias_initializer="zeros", activation="relu"),
        Dense(units=latent_dim*2, kernel_initializer="normal", bias_initializer="zeros", activation="sigmoid")
    ])
    ...
    #decoder = build_deconv_net(latent_dim, activation_out='sigmoid', filters=filters)
    decoder = tf.keras.models.Sequential(layers=[
        tf.keras.Input(shape=(latent_dim,)),
        Dense(4 * 4 * filters * 2),
        LeakyReLU(0.2),
        Reshape((4, 4, filters * 2)),
        # 4x4 -> 8x8
        Conv2DTranspose(filters*2, 3, strides=2, padding='same'),
        BatchNormalization(),
        LeakyReLU(0.2),
        # 8x8 -> 16x16
        Conv2DTranspose(filters*2, 3, strides=2, padding='same'),
    ...
    # Define a custom layer for the KL loss calculation with warmup
    class KLLossLayer(tf.keras.layers.Layer):
        def __init__(self, kl_weight_var, **kwargs):
            super().__init__(**kwargs)
            self.kl_weight_var = kl_weight_var  # Store reference to the variable
        ...
        def call(self, inputs):
            ...
            kl_loss = -0.5 * tf.reduce_sum(1 + z_var - tf.square(z_mean) - tf.exp(z_var))
            # Normalize by the input size
            kl_loss = kl_loss / tf.cast(tf.keras.backend.prod(data_shape), tf.float32)
            # Apply the dynamic warmup weight (this reads the variable value dynamically)
            weighted_kl_loss = self.kl_weight_var * kl_loss
            self.add_loss(weighted_kl_loss)
            
            return inputs

    # Apply the custom layer to z_mean and z_var
    # Pass the variable itself, not its evaluated value
    _, _ = KLLossLayer(kl_weight_var)([z_mean, z_var])

    vae.compile(optimizer=tf.keras.optimizers.Adam(learning_rate=1e-3), loss='huber')

    # Create the warmup callback
    kl_warmup = KLWarmup(kl_weight_var, warm_up_epochs, max_kl_weight=max_kl_weight)

    return encoder, decoder, vae, kl_warmup

# %%
# Training the VAE model

latent_dim = 16
encoder, decoder, vae, warmup_callback = build_vae(dataset.shape[1:], latent_dim, filters=256, warm_up_epochs=15, max_kl_weight=0.6)
...
vae.fit(x=dataset, y=dataset, epochs=40, batch_size=8, callbacks=[warmup_callback, image_generation, lr_reduction, early_stop])

# for epoch in range(20):
#     vae.fit(x=dataset, y=dataset, epochs=1, batch_size=8, callbacks=[warmup_callback])
...
# %%
def interpolate(latent_dim, num_steps = 10):

    # Generate two random latent vectors
    latent_vectors = np.random.randn(2, latent_dim)
    za = latent_vectors[0:1]  # Shape (1, latent_dim)
    zb = latent_vectors[1:2]  # Shape (1, latent_dim)
    interpolated_latents = []

    # Generate alpha values for linear interpolation
    ...
    latent_batch = np.vstack(interpolated_latents)  # Shape (num_steps, latent_dim)

    # Decode interpolated latents using decoder
    interpolated_outputs = decoder.predict(latent_batch)

    # Plot the interpolation
    fig, axes = plt.subplots(1, num_steps, figsize=(20, 2))
    for i, ax in enumerate(axes):
        ax.imshow(interpolated_outputs[i])
        ax.axis('off')
...
def train_gan(generator, discriminator, gan, dataset, latent_dim, n_epochs=20, batch_size=16):
    """
...
latent_dim = 16
discriminator, generator, gan = build_gan(dataset.shape[1:], latent_dim, filters=256)
dataset_scaled = load_real_samples(scale=True)

train_gan(generator, discriminator, gan, dataset_scaled, latent_dim, n_epochs=40, batch_size=8)

# %%
def interpolate_gan(generator, latent_dim, num_steps=10):
    """
    Interpolate between two random latent vectors in GAN
    """
    # Sample two random latent vectors
    za = np.random.randn(1, latent_dim)
    zb = np.random.randn(1, latent_dim)
    ...
    # Stack and generate images
    latent_batch = np.vstack(interpolated_latents)
    generated_images = generator.predict(latent_batch, verbose=0)
    
    # Scale from [-1, 1] to [0, 1] for visualization because of tanh
...
for _ in range(10):
    interpolate_gan(generator, latent_dim)
